  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Payment Methods */}
        <div className="card bg-surface-container-low border border-outline-variant/10 shadow-sm rounded-2xl p-6">
          <div className="space-y-4">
            <h3 className="text-lg font-bold text-on-surface mb-4">Payment Methods</h3>
            {isLoading ? (
              <div className="h-48 bg-surface-container-high rounded-xl animate-pulse"></div>
            ) : data?.paymentMethods && data.paymentMethods.length > 0 ? (
              <div className="space-y-3">
                {data.paymentMethods.map((method) => (
                  <div key={method._id} className="flex items-center justify-between p-4 bg-surface-container rounded-xl border border-outline-variant/10">
                    <span className="font-bold text-sm capitalize">{method._id}</span>
                    <div className="flex items-center gap-3">
                      <span className="text-lg font-bold text-on-surface">{method.count}</span>
                      <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-widest">{method.percentage.toFixed(1)}%</span>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="h-48 flex items-center justify-center text-muted-foreground font-bold">
        ...
        <div className="card bg-surface-container-low border border-outline-variant/10 shadow-sm rounded-2xl p-6">
          <div className="space-y-4">
            <h3 className="text-lg font-bold text-on-surface mb-4">Order Distribution by Hour</h3>
            {isLoading ? (
              <div className="h-48 bg-surface-container-high rounded-xl animate-pulse"></div>
                  ...
                  {data.hourlyDistribution.map((hour) => {
                    ...
                    return (
                      <div key={hour.hour} className="flex flex-col items-center flex-1">
                        <div
                          className="bg-primary hover:bg-primary-container transition-colors w-full rounded-t-lg"
                          style={{ height: `${height}%` }}
      ...
      {/* Footer */}
      <div className="text-center text-[10px] uppercase tracking-widest font-bold text-muted-foreground py-8 border-t border-outline-variant/10 mt-12">
