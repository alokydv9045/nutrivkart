...
// Common quantity variants (weights/servings)
const COMMON_VARIANTS = ['250g', '500g', '1kg', '2kg', '3kg', '5kg', '30 Servings', '60 Servings'];
        ...
        <div className='card bg-base-100 shadow-sm'>
          <div className='card-body p-5 space-y-4'>
            <div className='flex items-center justify-between'>
              <h2 className='font-semibold tracking-wide text-sm uppercase opacity-70'>Variants / Weights</h2>
              <div className='flex gap-2'>
                <input
                  type='text'
                  placeholder='Custom variant (e.g. 100 Tabs)'
                  value={customSize}
            ...
            <div className='flex flex-wrap gap-2'>
              ...
              {sizes.filter(s => !COMMON_VARIANTS.includes(s)).map(sz => (
                <button
                  key={sz}
                  type='button'
                  onClick={() => addOrRemoveSize(sz)}
            ...
            {sizes.length === 0 && <p className='text-[11px] opacity-70'>Select at least one variant (optional but recommended).</p>}
          </div>
        </div>
