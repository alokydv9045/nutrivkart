...
export const POST = auth(async (req: any) => {
  ...
  try {
    ...
    if (paymentDetails.amount !== expectedAmountPaise) {
      console.error(`[RAZORPAY FRAUD ATTEMPT] Order ${orderId} expected ${expectedAmountPaise} paise but received ${paymentDetails.amount} paise.`);
      return Response.json(
        { message: 'Payment amount mismatch. Order verification failed.' },
        { status: 400 }
      );
    }
