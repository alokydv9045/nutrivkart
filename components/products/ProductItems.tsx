...
const ProductItems = async ({ 
  ...
  if (products.length === 0) {
    return (
      <div className='rounded-sm bg-surface-container p-6 text-center text-muted-foreground font-medium'>
        No products found.
      </div>
    );
  }
    ...
    <div className="w-full">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="flex items-center justify-between mb-6 md:mb-8">
          <h2 className="text-2xl sm:text-[28px] md:text-[32px] font-black tracking-tight uppercase text-foreground">
            {title}<span className="ml-2 text-primary">{highlight}</span>
          </h2>
          <Link 
            href="/shop" 
            className="flex items-center gap-1 text-sm font-bold uppercase tracking-widest text-foreground hover:text-primary transition-colors group shrink-0"
          >
            View all
    ...
    <div className="w-full">
      ...
      <div className={containerClasses}>
        {Array.from({ length: qty }).map((_, i) => (
          <div 
            key={i} 
            className={`${layout === 'slider' ? 'w-[280px] shrink-0' : ''} bg-surface-container border border-border rounded-sm animate-pulse flex flex-col space-y-4 p-4 aspect-[4/5]`}
          >
            <div className='w-full h-[60%] bg-surface-bright rounded-sm' />
            <div className='h-6 bg-surface-bright rounded-sm w-3/4' />
            <div className='h-4 bg-surface-bright rounded-sm w-1/4' />
          </div>
        ))}
      </div>
    </div>
