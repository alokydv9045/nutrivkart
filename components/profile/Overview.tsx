...
export default function Overview({ user, onUpdateAvatar, onSaveAbout }: Props) {
  ...
  const { data: orders } = useSWR<OrderLite[]>('/api/orders/mine', (url) => fetcher(url), { refreshInterval: 5000 });
  const latestOrder = orders?.[0];

  const { data: searchData } = useSWR<any>('/api/products/search?q=all', (url) => fetcher(url), { refreshInterval: 60000 });
  const recommendedProducts = searchData?.products?.slice(0, 4) || [];
    ...
    <div className="space-y-12 animate-reveal">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 relative z-10">
        ...
        <div className="lg:col-span-8 space-y-12">
          ...
          <section className="bg-surface-container-low/80 backdrop-blur-md border border-surface-variant/40 rounded-3xl p-8 shadow-sm hover:shadow-md transition-all">
            ...
            {latestOrder && latestOrder.orderItems?.[0] && (
              <div className="mt-10 flex items-center gap-6 p-5 bg-surface-container-high/50 backdrop-blur-sm rounded-2xl border border-outline-variant/10 group">
        ...
        <div className="lg:col-span-4 space-y-10">
          ...
          {/* Quick-Reorder Vault */}
          <section className="bg-surface-container-low border border-outline-variant/20 rounded-3xl p-8 shadow-sm">
              ...
              {Array.isArray(orders) && (
                orders.flatMap(o => o.orderItems || []).slice(0, 3).map((item, i) => (
                  <div key={i} className="flex items-center gap-3 group p-2 hover:bg-primary/5 rounded-xl transition-all cursor-pointer">
                    ...
                    <button className="w-8 h-8 flex items-center justify-center border border-outline-variant/20 rounded-full bg-surface shadow-sm hover:bg-primary hover:text-on-primary transition-all hover:scale-110 flex-shrink-0">
              ...
              {isEditing ? (
                ...
                >
                  <div className="space-y-2">
                    ...
                    <input 
                      className="w-full bg-surface border border-outline-variant/20 rounded-xl px-4 py-3 font-body focus:border-primary transition-colors outline-none text-on-surface"
                      value={name} 
      ...
      <section className="mt-32 pt-24 border-t border-primary/10 relative">
        ...
        {/* Dynamically fetched recommended products */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 relative z-10">
          {recommendedProducts.length > 0 ? recommendedProducts.map((item: any, i: number) => (
            <motion.div 
              key={item.slug || i} 
              whileHover={{ y: -10 }}
            ...
            >
              <div className="relative aspect-square mb-6 overflow-hidden bg-surface-container-low rounded-2xl shadow-xl shadow-primary/5">
                ...
                <div className="absolute bottom-6 left-6 right-6 translate-y-8 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                  <Link href={`/product/${item.slug}`} className="w-full block text-center bg-surface-container-high/90 backdrop-blur-md py-3 rounded-xl text-[9px] font-bold uppercase tracking-widest text-primary shadow-lg border border-outline-variant/10 hover:bg-primary hover:text-on-primary transition-all">
                    View Ritual
                  </Link>
                </div>
              </div>
            ...
            <div className="col-span-1 sm:col-span-2 lg:col-span-4 flex items-center justify-center min-h-[300px]">
